...
/*
   function fisherYates(imgArray) {
      var i = imgArray.length, j, tempi, tempj;
      if ( i === 0 ) return false;
      while ( --i ) {
   ...
   function startCycle() {
      ...
      for(i=0; i<(imgArray.length); i++) { 
         ...
         var filePath = 'voids/' + (imgArray[i]) + '.jpg';
         //console.log(filePath);
         image_preload.src = filePath;
         $('<img/>').attr({src: filePath, width: '1600px', height: '900px', style: "display:none", id: i}).appendTo($("#wrap"));
      }; 
      $("#wrap img").eq(3).load(function() {
         $('#wrap').cycle();
      });
   }; 
*/
...
/* 		    radial.addColorStop(.8, 'rgba(0,0,0,.6)'); */
